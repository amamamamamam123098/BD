...
local function pre_process(msg)
	...
	if expiretime then    
		...
		rem_mutes(msg.to.id)
		superrem2(msg)
		return send_large_msg(get_receiver(msg), '⏰ تاریخ انقضای گروه شما به پایان رسید . برای تمدید به @SeyedRobot مراجعه کنید !')
		else
			return
		end
	end
...
..'----------------------------------\n'
			--local sends = send_msg(user, exppm, ok_cb, false)
			send_large_msg("channel#id"..1081597005, exppm)
